...
double new_abs(double num);				    //取绝对值
...
void putBall()   //目的：每次都找满足条件的最大球
{
    ...
    if(judge(maxBall))
    {
        ...
        printf("第%d\t %.3lf\t %.3lf\t %.3lf\t %lf \n", num, maxBall.x, maxBall.y, maxBall.r, sumr);
    }
}

int judge(Ball b)  //判断球是否
{
    //判断自己有没有越界

    if(((new_abs(b.x) + b.r) > 1.0) || ((new_abs(b.y) + b.r) > 1.0))
    {
        return 0;
    }

    BallList *tmp = head;
    //循环遍历整个链表 判断是否有球与球之间冲突的情况
    while(tmp)
    {
        Ball ball = tmp->ball;
